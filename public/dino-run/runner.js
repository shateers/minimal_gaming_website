...
Runner.prototype = {
  ...
  loadSounds: function() {
    if (!IS_IOS) {
      ...
      if (resourceTemplate) {
        const soundSources = resourceTemplate.querySelectorAll('audio');

        for (let i = 0; i < soundSources.length; i++) {
          const soundSource = soundSources[i];
          const id = soundSource.id;
          const src = soundSource.src;
          if (src) {
            this.sandboxController.fetchAndDecode(src, (buffer) => {
              this.soundFx[id] = buffer;
            });
          }
        }
      }
  ...
  init: function() {
    ...
    this.canvas = /** @type {HTMLCanvasElement} */
        (document.getElementById('runner-canvas'));

    if (!this.canvas) {
      this.canvas = /** @type {HTMLCanvasElement} */
          (document.createElement('canvas'));
      this.canvas.className = Runner.classes.CANVAS;
  ...
  adjustDimensions: function() {
    ...
    if (this.isArcadeMode()) {
      ...
      if (this.dimensions.WIDTH < DEFAULT_WIDTH) {
        this.dimensions.HEIGHT = Math.round(this.dimensions.WIDTH *
            this.dimensions.HEIGHT / DEFAULT_WIDTH);
      }
    }
  ...
  /**
   * Polls for a gamepad with buttons pressed. If a gamepad is found with
   * buttons down, it is treated as the active gamepad and all others are
   * ignored.
   * @param {!Array} gamepads
   */
  pollForActiveGamepad: function(gamepads) {
    for (let i = 0; i < gamepads.length; ++i) {
      if (gamepads[i] && gamepads[i].buttons.some(function(e) {
        return e.pressed;
      })) {
        this.gamepadIndex = i;
        this.pollActiveGamepad(gamepads);
  ...
  pollActiveGamepad: function(gamepads) {
    ...
    // The gamepad specification defines buttons and axes as arrays of
    // GamepadButton and GamepadAxisMoveEvent objects respectively. Here, we use
    // only buttons for game control.
    const buttonStates = gamepad.buttons.map(function(e) {
      return e.pressed;
    });

    // Generating events for the currently pressed buttons.
    const downButtons = buttonStates.filter(function(e) {
      return e ? true : false;
    });

    const gamepadToKeyMap = {
      0: 38,  // Jump
      1: 40   // Duck
    };

    downButtons.forEach(function(_, i) {
      if (buttonStates[i] != this.previousGamepad[i]) {
        const evt = new KeyboardEvent(
            buttonStates[i] ? Runner.events.KEYDOWN : Runner.events.KEYUP, {
              keyCode: gamepadToKeyMap[i]
            });
        document.dispatchEvent(evt);
      }
    }.bind(this));
    this.previousGamepad = buttonStates;
  },
 ...
 * @param {Element=} opt_container The canvas container.
 * @param {number=} opt_zIndex Canvas z-index.
 * @return {HTMLCanvasElement}
 */
function createCanvas(opt_container, opt_zIndex) {
  const canvas = /** @type {!HTMLCanvasElement} */ (document.createElement('canvas'));
  canvas.className = Runner.classes.CANVAS;
  canvas.width = DEFAULT_WIDTH;
  canvas.height = 150;
  canvas.style.zIndex = opt_zIndex;

  if (opt_container) {
    opt_container.appendChild(canvas);
  }
...
NightMode.prototype = {
  ...
  placeStars: function() {
    ...
    for (let i = 0; i < NightMode.config.NUM_STARS; i++) {
      ...
      this.stars[i].y = getRandomNum(0, NightMode.config.STAR_SIZE * 2);
...
Horizon.prototype = {
  ...
  init: function() {
    ...
    this.nightMode = new NightMode(this.canvas, this.spritePos, this.dimensions.WIDTH);
  },
