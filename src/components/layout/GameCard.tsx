...
const GameCard = ({
  ...
  return <div ref={cardRef} className={`opacity-0 transform translate-y-10 ${isVisible ? "animate-slide-in" : ""}`} style={{
    animationDelay: `${index * 0.1}s`
  }}>
      <Link to={href} className="block" onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
        <div className={`relative rounded-xl overflow-hidden border border-gray-200 bg-white shadow-sm
          ...
          <div className="flex flex-col h-full p-5 bg-rose-300">
            <div className="mb-4 bg-gray-100 rounded-lg h-40 overflow-hidden flex items-center justify-center">
              {imageSource && !imageError ? <img src={imageSource} alt={title} className="w-full h-full object-contain transition-transform duration-700 ease-out" onError={e => {
              handleImageError(e);
              setImageError(true);
