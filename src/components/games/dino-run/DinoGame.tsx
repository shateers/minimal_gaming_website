...
const DinoGame: React.FC<DinoGameProps> = ({ onScoreUpdate, onGameOver }) => {
  ...
  useEffect(() => {
    if (gameLoaded && containerRef.current && window.Runner) {
      try {
        // First, make sure any previous instance is cleaned up
        const existingOfflineResources = document.getElementById('offline-resources');
        if (existingOfflineResources) {
          document.body.removeChild(existingOfflineResources);
        }

        // Create the offline resources container
        const offlineContainer = document.createElement("div");
        offlineContainer.id = "offline-resources";
        offlineContainer.style.display = "none"; // Make sure it's hidden
        ...
        const imagePromises = [
          new Promise((resolve) => { img1x.onload = resolve; }),
          new Promise((resolve) => { img2x.onload = resolve; })
        ];

        Promise.all(imagePromises).then(() => {
          // Initialize the game after images are loaded
          setTimeout(() => {
            if (containerRef.current && window.Runner) {
              // Initialize the game
          ...
          }, 300); // Increase delay to ensure everything is ready
        }).catch(error => {
